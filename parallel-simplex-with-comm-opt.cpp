...
void findColPhase2(IndexValuePair *cLocalMaxima, long &e, STATUS &status) {
...
void pivotPhase1(long s, long t, long eModN, long eDivN, long locRows, flt *c, flt *aie, flt **A, flt *c2) {
  if (eModN == t) {
    // Start two-phase broadcast of column e
    for (long i = 0; i < locRows; i++)
      aie[i] = A[i][eDivN];
    aie[locRows] = c[eDivN];
    if (c2 != nullptr) aie[locRows + 1] = c2[eDivN];
  }
  bsp_broadcast(aie, locRows + 2, s * N + eModN, s * N + 0, s * N + t, 1, N, 0);
}

void pivotPhase2(long s, long t, long eModN, long locRows, flt *aie) {
  bsp_broadcast(aie, locRows + 2, s * N + eModN, s * N + 0, s * N + t, 1, N, 1);
}

void pivotPhase3(
        long s, long t, long lModM, long lDivM, long locCols, long eModN, long eDivN, flt *aie, flt *alj, flt **A,
        flt *b, long *Basis, long *NonBasis, long e, long l) {
  ...
  bsp_broadcast(alj, locCols + 1, lModM * N + t, 0 * N + t, s * N + t, N, M, 0);
}

void pivotPhase4(long s, long t, flt *alj, long locCols, long lModM) {
  bsp_broadcast(alj, locCols + 1, lModM * N + t, 0 * N + t, s * N + t, N, M, 1);
}

void pivotPhase5(
        long s, long t, long locRows, long locCols, long eDivN, long eModN, long lModM, long lDivM,
        flt **A, const flt *alj, const flt *aie, flt *b, flt *c, flt &v, flt *c2, flt &v2) {
  for (long i = 0; i < locRows; i++) {
  ...
  for (long i = 0; i < locRows; i++) {
    if (lModM == s && i == lDivM) continue;
    b[i] = max(b[i] - aie[i] * alj[locCols], 0.); // Dont let b become infeasible! alj[locCols] == b[l]
  }

  for (long j = 0; j < locCols; j++) {
    if (eModN == t && j == eDivN)
      c[j] = -aie[locRows] * alj[j]; // aie[locRows] == c[e]
    else
      c[j] -= aie[locRows] * alj[j]; // aie[locRows] == c[e]
  }
  if (c2 != nullptr) {
    for (long j = 0; j < locCols; j++) {
      if (eModN == t && j == eDivN)
        c2[j] = -aie[locRows + 1] * alj[j]; // aie[locRows + 1] == c2[e]
      else
        c2[j] -= aie[locRows + 1] * alj[j]; // aie[locRows + 1] == c2[e]
    }
  }
  v += aie[locRows] * alj[locCols];
  if (c2 != nullptr) v2 += aie[locRows + 1] * alj[locCols];
}
...
result simplex_slack(long s, long t, long m, long n, flt **A, flt *c, flt *b, flt &v,
                     ...
                     STATUS &status, IndexValuePair *baLocalMinima,
                     flt *aie, flt *alj, long &iterations, flt *c2, flt &v2) {
  ...
  while (true) {
    ...
    findColPhase2(cLocalMaxima, e, status);
    ...
    pivotPhase1(s, t, eModN, eDivN, locRows, c, aie, A, c2);
    ...
    pivotPhase3(s, t, lModM, lDivM, locCols, eModN, eDivN, aie, alj, A, b, Basis, NonBasis, e, l);
    ...
    pivotPhase5(s, t, locRows, locCols, eDivN, eModN, lModM, lDivM, A, alj, aie, b, c, v, c2, v2);
    bsp_sync();
...
result simplex(long M, long N, long s, long t, long m, long n, flt **A, flt *c, flt *b,
  ...
  flt *aie = new flt[auxLocRows + 2]; // For c1[e], c2[e]
  flt *alj = new flt[auxLocCols + 1]; // For b[l]
  STATUS status = RUNNING;
  long iterations = 0;

  bsp_push_reg(aie, (auxLocRows + 2) * sizeof(flt));
  bsp_push_reg(alj, (auxLocCols + 1) * sizeof(flt));
  ...
  if (l == -1) { // We can use the canonical slack form initially.
    ...
    if (DEBUG) printf("%li, %li: Pivot auxiliary variable into the basis, l=%li.\n", s, t, l);

    pivotPhase1(s, t, n % N, n / N, auxLocRows, auxc, aie, auxA, c);
    bsp_sync();
    pivotPhase2(s, t, n % N, auxLocRows, aie);
    bsp_sync();
    pivotPhase3(s, t, l % M, l / M, auxLocCols, n % N, n / N, aie, alj, auxA, b, Basis, NonBasis, n, l);
    bsp_sync();
    pivotPhase4(s, t, alj, auxLocCols, l % M);
    bsp_sync();
    pivotPhase5(s, t, auxLocRows, auxLocCols, n / N, n % N, l % M, l / M, auxA, alj, aie, b, auxc, auxv, c, v);
    bsp_sync();
    iterations++;
    if (DEBUG) printf("%li, %li: Pivot succeeded. Solving auxiliary problem.\n", s, t);

    l = -1;
    result auxRes = simplex_slack(s, t, m, n + 1, auxA, auxc, b, auxv, Basis, NonBasis, cLocalMaxima, e, l,
                                  status, baLocalMinima, aie, alj, iterations, c, v);
    ...
    for (l = 0; l < m; l++) {
      if (Basis[l] == n + m) {
        ...
        e = -1;
        if (l % M == s) {
          if (t == 0 && DEBUG) printf("Erasing the pivot to keep solution feasible: b[l]=%.17g\n", b[l / M]);
          b[l / M] = 0;
          long localIndex = -1;
          flt localMax = 0.;
          for (long j = 0; j < auxLocCols; j++) {
        ...
        pivotPhase1(s, t, e % N, e / N, auxLocRows, auxc, aie, auxA, c);
        bsp_sync();

        pivotPhase2(s, t, e % N, auxLocRows, aie);
        bsp_sync();

        pivotPhase3(s, t, l % M, l / M, auxLocCols, e % N, e / N, aie, alj, auxA, b, Basis, NonBasis, e, l);
        bsp_sync();

        pivotPhase4(s, t, alj, auxLocCols, l % M);
        bsp_sync();

        pivotPhase5(s, t, auxLocRows, auxLocCols, e / N, e % N, l % M, l / M, auxA, alj, aie, b, auxc, auxv, c, v);
        bsp_sync();
    ...
    if (remCol % N == n % N) {
      ...
      if (t == remCol % N) {
        for (long i = 0; i < auxLocRows; i++)
          aie[i] = A[i][remCol / N];
        aie[auxLocRows] = c[remCol / N];
        bsp_put(s * N + (n % N), aie, aie, 0, (auxLocRows + 1) * sizeof(flt));
      } else if (t == n % N) {
        for (long i = 0; i < auxLocRows; i++)
          aie[i] = auxA[i][n / N];
        aie[auxLocRows] = c[n / N];
        bsp_put(s * N + (remCol % N), aie, aie, 0, (auxLocRows + 1) * sizeof(flt));
      }
      bsp_sync();
      if (t == remCol % N) {
        c[remCol / N] = aie[auxLocRows];
        for (long i = 0; i < auxLocRows; i++)
          auxA[i][remCol / N] = aie[i];
      } else if (t == n % N) {
        c[n / N] = aie[auxLocRows];
        for (long i = 0; i < auxLocRows; i++)
          auxA[i][n / N] = aie[i];
      }
  ...
  result res = simplex_slack(s, t, m, n, A, c, b, v, Basis, NonBasis, cLocalMaxima, e, l, status, baLocalMinima,
                             aie, alj, iterations, nullptr, v);
  ...
  if (s == 0 && t == 0) {
    ...
    for (long i = 0; i < M; i++) {
      for (long j = 0; j < N; j++) {
        ...
        bsp_put(i * N + j, lb[i], b, 0, nloc(M, i, m) * sizeof(flt));
      }
    }
    for (long j = 0; j < N; j++)
      for (long i = 0; i < M; i++)
        bsp_put(i * N + j, lc[j], c, 0, nloc(N, j, n) * sizeof(flt));
